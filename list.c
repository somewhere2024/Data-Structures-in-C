...
list* list_create(void)
{
    ...
    list->head = list->tail = NULL;   //初始化链表 
    list->length = 0;
    return list;
}
...
list_iter* list_get_iterator(list* list, int direction)
{
    ...
    if (direction == LIST_START_HEAD)  //判断方向
        iter->next = list->head;
    else
        iter->next = list->tail;
...
list* list_add_node_head(list* list, void* value)
{
    ...
    if (list->length == 0) {
        ...
        node->prev = NULL;  //设置新节点
        node->next = list->head;
        list->head->prev = node;  //设置链表
        list->head = node;
    }
    list->length++;
    return list;
...
void* list_get_value_head(list* list)
{
    ...
    void* value = list->head->value;   //临时创建一个变量保存数据
    list_del_node(list, list->head);   //删除该节点
    return value;
}
